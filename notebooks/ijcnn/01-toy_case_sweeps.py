...
def shuffled_blobs(N, D, centers, blob_std, random_state = None):
    ...
    assert samples_per_center * n_centers == N, "N must be divisible by number of centers"
...
if __name__ == "__main__":
    def run_all_sweeps(N, D):
        ...
        sim_blobs = lib_get_embeddings.calculate_all_similaritiees(X, Y, lib_get_embeddings.similarities)
        ...
        sim_shuffled = lib_get_embeddings.calculate_all_similaritiees(X, Y, lib_get_embeddings.similarities)
        nngs_shuffled, ks = nngs_sweeep(X, Y)
        
        
        X, Y = np.random.normal(size=(N, D)), np.random.normal(size=(N, D))
        sim_noise = lib_get_embeddings.calculate_all_similaritiees(X, Y, lib_get_embeddings.similarities)
        nngs_noise, ks = nngs_sweeep(X, Y)
        
        return ks, nngs_blobs, nngs_shuffled, nngs_noise, sim_blobs, sim_shuffled, sim_noise
    
    ks5, nngs_blobs5, nngs_shuffled5, nngs_noise5, sim_blobs5, sim_shuffled5, sim_noise5 = run_all_sweeps(N=500, D=10)
    ks50, nngs_blobs50, nngs_shuffled50, nngs_noise50, sim_blobs50, sim_shuffled50, sim_noise50 = run_all_sweeps(N=500, D=50)
    ks100, nngs_blobs100, nngs_shuffled100, nngs_noise100, sim_blobs100, sim_shuffled100, sim_noise100 = run_all_sweeps(N=500, D=100)   
    ks2000, nngs_blobs2000, nngs_shuffled2000, nngs_noise2000, sim_blobs2000, sim_shuffled2000, sim_noise2000 = run_all_sweeps(N=500, D=2000)       
    ...
    plt.plot(ks50, nngs_blobs50, color=colors[0], linestyle='--', label=f"{experiment1}, D=50", alpha=0.7)
    plt.plot(ks100, nngs_blobs100, color=colors[0], linestyle='-.', label=f"{experiment1}, D=100", alpha=0.7)
    plt.plot(ks2000, nngs_blobs2000, color=colors[0], linestyle=':', label=f"{experiment1}, D=2000", alpha=0.7)
    plt.plot(ks5, nngs_shuffled5, color=colors[1],label=f"{experiment2}, D=10", alpha=0.7)
    plt.plot(ks50, nngs_shuffled50, color=colors[1], linestyle='--', label=f"{experiment2}, D=50", alpha=0.7)
    plt.plot(ks100, nngs_shuffled100, color=colors[1], linestyle='-.', label=f"{experiment2}, D=100", alpha=0.7)
    plt.plot(ks2000, nngs_shuffled2000, color=colors[1], linestyle=':', label=f"{experiment2}, D=2000", alpha=0.7)
    plt.plot(ks5, nngs_noise5, color=colors[2], label=f"{experiment3}, D=10", alpha=0.7)
    plt.plot(ks50, nngs_noise50, color=colors[2], linestyle='--', label=f"{experiment3}, D=50", alpha=0.7)
    plt.plot(ks100, nngs_noise100, color=colors[2], linestyle='-.', label=f"{experiment3}, D=100", alpha=0.7)
    plt.plot(ks2000, nngs_noise2000, color=colors[2], linestyle=':', label=f"{experiment3}, D=2000", alpha=0.7)

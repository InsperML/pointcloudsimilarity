...
if __name__ == "__main__":
    def run_all_sweeps(N, D):
        ...
        sim_blobs = calculate_all_similaritiees(X, Y, similarities)
        ...
        sim_shuffled = calculate_all_similaritiees(X, Y, similarities)
        nngs_shuffled, ks = nngs_sweeep(X, Y)
        
        
        X, Y = np.random.normal(size=(N, D)), np.random.normal(size=(N, D))
        sim_noise = calculate_all_similaritiees(X, Y, similarities)
    ...
    plt.figure(figsize=(config['width']*2, config['height']))
    experiment1 = "Local Jitter"
    experiment2 = "Shuffled Centers"
    experiment3 = "Random Noise"
    plt.plot(ks5, nngs_blobs5, 'b', label=f"{experiment1}, D=10", alpha=0.7)
    plt.plot(ks5, nngs_shuffled5, 'r',label=f"{experiment2}, D=10", alpha=0.7)
    plt.plot(ks5, nngs_noise5, 'g', label=f"{experiment3}, D=10", alpha=0.7)
    plt.plot(ks50, nngs_blobs50, 'b:', label=f"{experiment1}, D=50", alpha=0.7)
    plt.plot(ks50, nngs_shuffled50, 'r:', label=f"{experiment2}, D=50", alpha=0.7)
    plt.plot(ks50, nngs_noise50, 'g:', label=f"{experiment3}, D=50", alpha=0.7)
    plt.plot(ks100, nngs_blobs100, 'b--', label=f"{experiment1}, D=100", alpha=0.7)
    plt.plot(ks100, nngs_shuffled100, 'r--', label=f"{experiment2}, D=100", alpha=0.7)
    plt.plot(ks100, nngs_noise100, 'g--', label=f"{experiment3}, D=100", alpha=0.7)
    plt.plot(ks2000, nngs_blobs2000, 'b-.', label=f"{experiment1}, D=2000", alpha=0.7)
    plt.plot(ks2000, nngs_shuffled2000, 'r-.', label=f"{experiment2}, D=2000", alpha=0.7)
    plt.plot(ks2000, nngs_noise2000, 'g-.', label=f"{experiment3}, D=2000", alpha=0.7)

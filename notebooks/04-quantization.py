...
def model_vs_quantized_model_similarities():
    ...
    l1, = ax1.plot(alphas_bert8,
                   np.array(similarities_cka_bert8),
                   label='bert-base-uncased (8 bits)')
    l2, = ax1.plot(alphas_bert4,
                   np.array(similarities_cka_bert4),
                   label='bert-base-uncased (4 bits)')
    l3, = ax1.plot(alphas_gpt8,
                   np.array(similarities_cka_gpt8),
                   label='gpt2 (8 bits)')
    l4, = ax1.plot(alphas_gpt4,
                   np.array(similarities_cka_gpt4),
                   label='gpt2 (4 bits)')
    l5, = ax1.plot(alphas_bertlarge8,
                   np.array(similarities_cka_bertlarge8),
                   label='bert-large-uncased (8 bits)')
    l6, = ax1.plot(alphas_bertlarge4,
                   np.array(similarities_cka_bertlarge4),
                   label='bert-large-uncased (4 bits)')
    ...
    ax2.plot(ks_bert8, np.array(similarities_nngs_bert8))
    ax2.plot(ks_bert4, np.array(similarities_nngs_bert4))
    ax2.plot(ks_gpt8, np.array(similarities_nngs_gpt8))
    ax2.plot(ks_gpt4, np.array(similarities_nngs_gpt4))
    ax2.plot(ks_bertlarge8, np.array(similarities_nngs_bertlarge8))
    ax2.plot(ks_bertlarge4, np.array(similarities_nngs_bertlarge4))
    ...
    fig.suptitle(
        f'Model Similarity Sweep between model and its quantized versions\\n(N={N_SAMPLES})')
    fig.tight_layout(rect=[0, 0.05, 1, 1])  # leave space for the legend
    fig.savefig(figname := 'model_similarity_quantized.png')
    plt.close()
